...
game::game() {
  ...
  gameUI.addElement(
      new Button(25, 25 + 64 + 4, "tweezer",
                 tools::load_bitmap_ex("images/tweezersButton.png")));
  ...
  entrance_front_transparent =
      tools::load_bitmap_ex("images/tiles/EntranceFrontTrans.png");
    ...
    while (getline(myfile, line)) {
      ...
      for (auto& s : tokens) {
        if (s == "100") {
          ...
          gameTiles.push_back(
              createTile(i, j - joffset, tools::convertStringToInt(s)));
...
bool game::canPlaceTile(int x, int y) {
  for (unsigned int i = 0; i < gameTiles.size(); i++) {
    if (x == gameTiles.at(i)->getX() && y == gameTiles.at(i)->getY()) {
      if (gameTiles.at(i)->getType() == 8 || gameTiles.at(i)->getType() == 2 ||
          gameTiles.at(i)->getType() == 9 || gameTiles.at(i)->getType() == 3)
        return false;
    }
  }
...
void game::update() {
  ...
  for (unsigned int i = 0; i < gameTiles.size(); i++) {
    ...
    if ((mouseListener::mouse_button & 1 && !gameUI.isHovering()) &&
        canPlaceTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY()) &&

        mxo > gameTiles.at(i)->getIsoX() - 32 &&
        mxo < gameTiles.at(i)->getIsoX() + 32 &&
        myo < gameTiles.at(i)->getIsoY() + 32 &&
        myo > gameTiles.at(i)->getIsoY() - 32) {
      switch (editor_tool) {
        case 0:
          gameTiles.at(i) =
              createTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY(), 5);
          money -= 100;
          editor_tool = 4;
          break;
        ...
        case 1:
          gameTiles.at(i) =
              createTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY(), 6);
          money -= 100;
          editor_tool = 4;
        ...
        case 2:
          gameTiles.at(i) =
              createTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY(), 7);
          money -= 100;
        ...
        case 3:
          gameTiles.at(i) =
              createTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY(), 4);
          money -= 100;
        ...
        case 5:
          gameTiles.at(i) =
              createTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY(), 10);
          money -= 500;
  ...
  for (unsigned int i = 0; i < gameEnemies.size(); i++) {
    ...
    if (gameEnemies.at(i)->getHealth() == 0) {
      gameParticles.push_back(createParticle(gameEnemies.at(i)->getX(),
                                             gameEnemies.at(i)->getY(), 4));
  ...
  for (unsigned int i = 0; i < gameGuests.size(); i++) {
    ...
    if (!selectedGuest && editor_tool == 4 && !selectedGuest &&
        tools::clicked(
            gameGuests.at(i)->getX() - 25, gameGuests.at(i)->getX() + 25,
            gameGuests.at(i)->getY() - 30, gameGuests.at(i)->getY() + 45)) {
    ...
    for (unsigned int j = 0; j < gameTiles.size(); j++) {
      ...
      if (gameTiles.at(j)->colliding_loose(guest_x, guest_y) &&
          guest_x < 1920 && guest_x > 0 && guest_y < 1080 && guest_y > 0) {
        off_map = false;
      }
          ...
          if (current == 8 && !is_cart) {
            if (!is_cart) {
              std::string stringyboi =
                  gameGuests.at(i)->getName() + " has died from drowining.";
    ...
    if (guest_alive) {
      for (unsigned int j = 0; j < gameEnemies.size(); j++) {
        if (tools::collision(
                gameGuests.at(i)->getX(), gameGuests.at(i)->getX() + 16,
                gameEnemies.at(j)->getX() + 100,
                gameEnemies.at(j)->getX() + 400, gameGuests.at(i)->getY(),
                gameGuests.at(i)->getY() + 54, gameEnemies.at(j)->getY() + 100,
                gameEnemies.at(j)->getY() + 200)) {
          if (!is_cart) {
            std::string stringyboi = gameGuests.at(i)->getName() +
                                     " has died from an angry octopus.";
            Message::sendMessage(stringyboi);
            gameParticles.push_back(createParticle(guest_x, guest_y, 1));
            guests_died_enemies++;

            if (gameGuests.at(i)->getVelocityX() != 0 &&
                gameGuests.at(i)->getVelocityY() != 0) {
              gameGuests.at(i)->setVelocityX(10);
              gameGuests.at(i)->setVelocityY(0);
              gameParticles.push_back(
                  createParticle(guest_x + 32, guest_y + 32, 3));
            }

          } else
            gameParticles.push_back(createParticle(guest_x, guest_y, 3));

          gameEnemies.at(j)->applyDamage(abs(gameGuests.at(i)->getVelocityX()) +
                                         abs(gameGuests.at(i)->getVelocityY()));
    ...
    if (gameTiles.at(i)->getType() == 2) {
      if (frame >= spawn_rate) {
        if (guest_spawn > 0 && started) {
          gameGuests.push_back(
              createGuest(gameTiles.at(i)->getIsoX() + 64 - 8,
...
void game::draw() {
  ...
  if (keyListener::key[ALLEGRO_KEY_G]) {
    for (unsigned int i = 0; i < gameTiles.size(); i++) {
      al_draw_rectangle(
          gameTiles.at(i)->getIsoX() + 32, gameTiles.at(i)->getIsoY() + 0,
          gameTiles.at(i)->getIsoX() + 96, gameTiles.at(i)->getIsoY() + 64,
          al_map_rgb(99, 33, 00), 1);
    }
  }
  ...
  for (unsigned int i = 0; i < gameTiles.size(); i++) {
    if (canPlaceTile(gameTiles.at(i)->getX(), gameTiles.at(i)->getY())) {
      if (editor_tool >= 0 && editor_tool <= 3 &&
          gameTiles.at(i)->colliding(mouseListener::mouse_x,
                                     mouseListener::mouse_y)) {
        al_draw_bitmap(path_hover, gameTiles.at(i)->getIsoX(),
                       gameTiles.at(i)->getIsoY(), 0);
      }

      if (editor_tool == 5 &&
          gameTiles.at(i)->colliding(mouseListener::mouse_x,
                                     mouseListener::mouse_y))
        al_draw_bitmap(coaster, gameTiles.at(i)->getIsoX() - 200,
                       gameTiles.at(i)->getIsoY() - 300, 0);
    }
  }
  ...
  if (selectedGuest != nullptr) {
    selectedGuest->draw();
    al_draw_bitmap(cursor_closed, mouseListener::mouse_x - 8,
                   mouseListener::mouse_y - 56, 0);
  }

  else if (editor_tool == 4) {
    al_draw_bitmap(cursor_open, mouseListener::mouse_x - 8,
                   mouseListener::mouse_y - 56, 0);
  }

  switch (editor_tool) {
    case 0:
      al_draw_bitmap(path[0], mouseListener::mouse_x - 64,
                     mouseListener::mouse_y - 32, 0);
      break;

    case 1:
      al_draw_bitmap(path[1], mouseListener::mouse_x - 64,
                     mouseListener::mouse_y - 32, 0);
      break;

    case 2:
      al_draw_bitmap(path[2], mouseListener::mouse_x - 64,
                     mouseListener::mouse_y - 32, 0);
      break;

    case 3:
      al_draw_bitmap(path[3], mouseListener::mouse_x - 64,
                     mouseListener::mouse_y - 32, 0);
      break;

    case 5:
      al_draw_bitmap(coaster, mouseListener::mouse_x - 238,
                     mouseListener::mouse_y - 319, 0);
      break;
  ...
  // Cursor pointer
  // al_draw_rectangle( mouseListener::mouse_x, mouseListener::mouse_y,
  // mouseListener::mouse_x + 3, mouseListener::mouse_y + 3, al_map_rgb( 255,
  // 255, 255), 3);
  ...
  //  al_draw_textf( font, al_map_rgb( 0, 0, 0), 10, 170, 0, "Guests in
  //  park:%u",(unsigned int)inpark); al_draw_textf( font, al_map_rgb( 0, 0, 0),
  //  10, 220, 0, "Guests rescued:%i",guests_rescued); al_draw_textf( font,
  //  al_map_rgb( 0, 0, 0), 10, 270, 0, "Guests died to
  //  enemies:%i",guests_died_enemies); al_draw_textf( font, al_map_rgb( 0, 0,
  //  0), 10, 320, 0, "Guests died to falling:%i",guests_died_falling);

  al_draw_textf(font, al_map_rgb(100, 0, 0), 30, 200, 0, "Money:%i", money);

  // al_draw_textf( font, al_map_rgb( 255, 255, 255), 600, 20, 0,
  // "Cost:100$",money);

  if (level == 1)
    al_draw_textf(
        font_small, al_map_rgb(0, 0, 0), 30, 260, 0, "Remaining Guests:%i",
        10 - (guests_rescued + guests_died_enemies + guests_died_falling));

  if (level == 2)
    al_draw_textf(
        font_small, al_map_rgb(0, 0, 0), 30, 260, 0, "Remaining Guests:%i",
        15 - (guests_rescued + guests_died_enemies + guests_died_falling));

  if (level == 3)
    al_draw_textf(
        font_small, al_map_rgb(0, 0, 0), 30, 260, 0, "Remaining Guests:%i",
        70 - (guests_rescued + guests_died_enemies + guests_died_falling));

  if (level == 4)
    al_draw_textf(
        font_small, al_map_rgb(0, 0, 0), 30, 260, 0, "Remaining Guests:%i",
        150 - (guests_rescued + guests_died_enemies + guests_died_falling));

  if (level == 5)
    al_draw_textf(
        font_small, al_map_rgb(0, 0, 0), 30, 260, 0, "Remaining Guests:%i",
        200 - (guests_rescued + guests_died_enemies + guests_died_falling));
